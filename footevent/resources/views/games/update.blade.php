...
<title>FootEvenT — Modifier match</title>
...
<div class="max-w-3xl mx-auto px-8 py-12">
  ...
  <h1 class="font-bebas text-5xl mb-2">
    Modifier un <span class="text-green-400">match</span>
  </h1>
  ...
  @if(session('success'))
    <div class="mb-4 flex items-center gap-3 px-5 py-4 bg-green-950 border border-green-800 rounded-2xl">
  ...
  @if(session('error'))
    <div class="mb-4 flex items-center gap-3 px-5 py-4 bg-red-950 border border-red-800 rounded-2xl">
      <p class="text-sm text-red-300 font-medium flex-1">
        {{ session('error') }}
      </p>
    </div>
  @endif
  ...
  <form method="POST" action="{{ route('games.update', $game) }}"
        class="space-y-6 bg-gray-900 border border-gray-800 rounded-2xl p-6">

    @csrf
    @method('PUT')

    <div>
      <label class="text-xs text-gray-400 uppercase mb-2 block">Date du match</label>
      <input type="date"
             name="dateMatch"
             value="{{ $game->dateMatch }}"
    ...
    <div>
      ...
      <input type="time" name="heure"value="{{ $game->heure }}"
             class="w-full bg-gray-950 border border-gray-700 rounded-xl px-4 py-3 text-sm">
    </div>

    <div>
      <label class="text-xs text-gray-400 uppercase mb-2 block">Terrain</label>
      <input type="text" name="terrain" value="{{ $game->terrain }}" class="w-full bg-gray-950 border border-gray-700 rounded-xl px-4 py-3 text-sm">
    </div>
